...
use Carbon\Carbon;
  ...
  $prev_period = Carbon::parse($pph21->pph21_period)->subMonth();
  $prev_periodm = $prev_period->format('m');
  $prev_periodmy = ($prev_periodm == 12) ? Carbon::parse($pph21->pph21_period)->format('m-Y') : $prev_period->format('m-Y');
  // dd($prev_period, $prev_periodm, $prev_periodmy);
?>
  ...
  <table>
    ...
    <tr>
      ...
      <th align="center">Penghasilan Netto Perusahaan Sebelumnya</th>
      <th align="center">PTKP</th>
      <th align="center">Penghasilan Kena Pajak Setahun</th>
      <th align="center">PPh Dipotong Sampai {{$prev_periodmy}}</th>
      <th align="center">PPh Dipotong Perusahaan Sebelumnya</th>
    ...
    <tbody>
      <?php foreach($data as $dt): 
        ...
        $pkptotal = ($pph21->pph21_pkp > 0) ? $pph21->pph21_pkp : 0;
        if($pph21->pph21_prevcp_netto > 0) {
          $pkptotal = $pph21->pph21_prevcp_netto + $pph21->pph21_netto_year - $pph21->pph21_ptkp;
        }
      ?>
        <tr>
          ...
          <td align="right" class="{{$isendperiod ? 'bg-grey' : '' }}">{{$isendperiod ? 'N/A' : $pph21->pph21_bruto_month}}</td>
          <td align="right" class="{{$isendperiod ? 'bg-grey' : '' }}">{{$isendperiod ? 'N/A' : $pph21->pph21_ptkpdet_rate_percentage}}</td>
          <td align="right" class="{{!$isendperiod ? 'bg-grey' : '' }}">{{!$isendperiod ? 'N/A' : $pph21->pph21_netto_month}}</td>
          <td align="right" class="{{!$isendperiod ? 'bg-grey' : '' }}">{{!$isendperiod ? 'N/A' : $pph21->pph21_netto_year}}</td>
          <td align="right" class="{{!$isendperiod ? 'bg-grey' : '' }}">{{!$isendperiod ? 'N/A' : $pph21->pph21_prevcp_netto}}</td>
          <td align="right" class="{{!$isendperiod ? 'bg-grey' : '' }}">{{!$isendperiod ? 'N/A' : $pph21->pph21_ptkp}}</td>
          <td align="right" class="{{!$isendperiod ? 'bg-grey' : '' }}">{{!$isendperiod ? 'N/A' : $pkptotal}}</td>
          <td align="right" class="{{!$isendperiod ? 'bg-grey' : '' }}">{{!$isendperiod ? 'N/A' : $pph21->pph21_total_year}}</td>
          <td align="right" class="{{!$isendperiod ? 'bg-grey' : '' }}">{{!$isendperiod ? 'N/A' : $pph21->pph21_prevcp_pph21_total}}</td>
