  ...
  ?>
    <div class="container pdf" id="pdf{{$i}}" data-kyname="{{$dt->payroll_karyawan_name.'_'.$dt->payroll_uuid}}">
      ...
      <div>
        <div class="row">
          <div class="col-sm-12">
            <h3 class="fs-16">C. NOMOR DOKUMEN REFERENSI FASILITAS ATAU SURAT KETERANGAN DOMISILI :</h3>
          </div>
        </div>
        <div>
          <div class="row">
            ...
            if (nextElementHTML == undefined) {
              zip.generateAsync({
                ...
                .then(function(content) {
                  // see FileSaver.js
                  saveAs(content, "buktipotong_bulanan_nonkaryawan_" + '<?php echo $filtermonth ?>' + ".zip");
                  // close window
                  setTimeout(function () {
                      window.close();
                  }, 1000)
