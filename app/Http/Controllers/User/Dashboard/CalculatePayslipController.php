...
class CalculatePayslipController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $data = [
            'title' => 'Kalkulasi Pasylip Karyawan',
            'content' => 'user.dashboard.calculate-payslip-karyawan',
        ];
    ...
    public function select_wajibpajak(Request $request)
    {
        ...
        $data = WajibPajakModel::when($q, function ($query, $q) {
    ...
    //     $periode = Carbon::createFromFormat('!m-Y', $request->get('periode'))->format('Y-m');
    ...
    //     ])
    //     ->with(['karyawan' => function($q) use($stpenggajian_id, $period, $date, $maxquota) {
    ...
    //         $q->limit($maxquota);
    //     }, 'karyawan.payroll' => function($q) use ($period, $maxquota) {
    ...
    //                 $temp_data = $penggajianController->calculatepayslip([
    ...
    public function calculate(Request $request)
    {
        ...
        //     return response()->json( [
        //         "success" => $decode_content,
        //         // "response" => (isset($decode_content->response) && $decode_content->response) ? json_decode($this->stringDecrypt($key, $decode_content->response)) : $decode_content->metaData,
        //     ]);
        // } catch(ClientException $e) {
        //     $response = $e->getResponse();
        //     $responseBodyAsString = $response->getBody()->getContents();
        //     $decodeResponseBodyAsString = json_decode($responseBodyAsString);
        //     // $responseBodyAsString = $response->getContents();
        //     // $decodeResponseBodyAsString = json_decode($responseBodyAsString);
        //     return response()->json( [
        //         "success" => false,
        //         "message" => $decodeResponseBodyAsString->message,
        //     ]);
        ...
        if($request->get('payroll_date_period')) {
            ...
            //     $payroll_date = date('d', strtotime($payroll_date_period));
            // } else {
            if($request->get('periodtype') == 'TANGGAL') {
            ...
            // dd($payroll_date, $payroll_period, $periode_year);
        } else {
        ...
        // END GET MAX QUOTA FROM SUBSCRIPTION
        // dd(count($currentstpenggajian->karyawan));
