...
class ProvinceController extends Controller
{
    ...
    public function select(Request $request)
    {
        ...
        $data = RegencyModel::select('ms_province.province_id', 'ms_province.province_name')
            ->join('ms_province', 'ms_province.province_id', '=', 'ms_regencies.province_id')
            ->when($q, function ($query, $q) {
                return $query->where('ms_province.province_name', 'ilike', '%' . $q . '%');
            })
            ->where($where)
            ->distinct()
            ->limit($limit)
