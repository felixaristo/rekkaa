...
class SettingPotonganKaryawanModel extends Model
{
    use LogsActivity;
    protected static $logOnlyDirty = true;

    protected $table = 'st_potongan_karyawan';
    protected $primaryKey = 'stpotongankaryawan_id';
    public $timestamps = false;

    protected static $logName = 'potongan_karyawan';
    protected static $logUnguarded = true;
    protected $labelTableName = 'st_potongan_karyawan';
    ...
    public function getDescriptionForEvent(string $eventName): string
    {
        ...
        if($eventName == 'created') {
            return "$defaultCauserName menambahkan pengaturan potongan baru dengan ID PO";
        } else {
            return "$defaultCauserName melakukan perubahan data pengaturan potongan dengan ID PO";
        }

        // return "[$defaultCauserType] $defaultCauserName, {$eventName} by: " . $defaultCauserName;
    }

    function stgrouppotongan() {
        return $this->belongsTo('App\Model\Setting\SettingGroupPotonganKaryawanModel', 'st_grouppotongankaryawan_id', 'stgrouppotongankaryawan_id');
    }

    function stpotongandetail() {
        return $this->hasMany('App\Model\Setting\SettingPotonganKaryawanDetailModel', 'st_potongankaryawan_id', 'stpotongankaryawan_id');
    }

    function stpotongandetailaktif() {
        return $this->hasMany('App\Model\Setting\SettingPotonganKaryawanDetailModel', 'st_potongankaryawan_id', 'stpotongankaryawan_id')->where(['stpotongankaryawandet_active' => '1']);
    }
