...
class SubscriptionModel extends Model
{
    ...
    protected $table = 'ms_subscription';
    protected $primaryKey = 'subscription_id';
    public $timestamps = false;
    
    protected static $logName = 'subscription';
    protected static $logUnguarded = true;
    protected $labelTableName = 'ms_subscription';
    ...
    public function getDescriptionForEvent(string $eventName): string
    {
        ...
        if($eventName == 'created') {
            return "$defaultCauserName menambahkan subscription baru dengan ID ";
        } else {
            return "$defaultCauserName melakukan perubahan data subscription dengan ID ";
        }

        // return "[$defaultCauserType] $defaultCauserName, {$eventName} by: " . $defaultCauserName;
    }

    // function subscriptionpermission() {
    //     return $this->hasMany('App\Model\Master\SubscriptionPermissionModel', 'ms_subscription_id', 'subscription_id')
    //     ->where(['subscriptionpermission_active' => 1]);
    // }

    function subscriptionpermission() {
        return $this->hasMany('App\Model\MasterRelation\MrSubscriptionPermissionModel', 'ms_subscription_id', 'subscription_id')
        ->where(['subscriptionpermission_active' => 1]);
    }
}
